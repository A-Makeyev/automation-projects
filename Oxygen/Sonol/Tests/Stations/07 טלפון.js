...
mob.transaction('06. Open Phone Number For Current Location')
utils.click(stationsPage.firstStationPhoneButton)

if (mob.isVisible('//android.widget.EditText[contains(@text, "")]')) {
    var number = mob.getText('//android.widget.EditText[contains(@text, "")]')
    log.info(`${number} :המסך הועבר להתקשר למספר`)
} else {
