...
function createNewLead(firstName, lastName, phone) {
    ...
    if (!web.isVisible('//input[@data-value="BBL"]', po.shortWait)) {
        po.click(leads.handlingCompanyBtn)
        if (web.isVisible(leads.handlingCompany.bezeqInternational, po.shortWait)) {
            po.click(leads.handlingCompany.bezeqInternational)
        }  else {
            po.click(leads.handlingCompany.bezeqInternational_hebrew)
        }      
    }

    po.click(leads.interestedInBtn)
    po.click(leads.interestedIn.internet)
