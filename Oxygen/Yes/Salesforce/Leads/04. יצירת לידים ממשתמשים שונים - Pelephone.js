...
function createNewLead(firstName, lastName, phone) {
    ...
    if (!web.isVisible('//input[@data-value="PEL"]', po.shortWait)) {
        po.click(leads.handlingCompanyBtn)
        if (web.isVisible(leads.handlingCompany.pelephone, po.shortWait)) {
            po.click(leads.handlingCompany.pelephone)
        }  else {
            po.click(leads.handlingCompany.pelephone_hebrew)
        }      
    }

    web.pause(po.waitASecond)
    po.click(leads.interestedInBtn)
    po.click(`${leads.interestedInBtn}//..//..//..//span[@data-option-index="${func.generateNumber(1, 5)}"]`)
