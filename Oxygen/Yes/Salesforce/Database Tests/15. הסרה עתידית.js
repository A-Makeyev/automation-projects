...
`
    SELECT DISTINCT O.ACCOUNT_NUMBER, o.addl_offer_id, M.OFFER_DESC, ot.offer_type_code,
        ot.offer_type_desc, cho.outlet_code outlet 
...
web.transaction('11. Select Future Date & Remove Reason')
web.execute(() => {
    document.evaluate(
        '//input[@name="Remove_Date"]', document, null,
...
web.execute(() => {
    document.evaluate(
        '//button[@title="החודש הבא"]', document, null,
        XPathResult.FIRST_ORDERED_NODE_TYPE, null
    ).singleNodeValue.click()
})
...
if (!web.isVisible(`//table[contains(@lightning-calendar, "")]//span[text()="${currentDay}"]`)) {
    po.click('//table[contains(@lightning-calendar, "")]//span[text()="25"]')
} else {
    web.execute((day) => {
        document.evaluate(
            `//table[contains(@lightning-calendar, "")]//span[text()="${day}"]`,
...
// close modal
po.click('//div[contains(@class, "slds-modal")]//*[name()="svg" and @data-key="close"]')
